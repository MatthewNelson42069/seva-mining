...
async def test_draft_for_post():
    """
    Mock Claude returns 2-3 comment alternatives; none contain hashtags (#).
    Covers: INST-05, INST-06
    """
    pytest.skip("Instagram agent not yet implemented")
    ia = _get_instagram_agent()
    mock_client = AsyncMock()
    mock_client.messages.create = AsyncMock(return_value=MagicMock(
        content=[MagicMock(text='["Gold demand from central banks reached a 40-year high.", '
                                '"Physical gold ETF inflows up 18% month-over-month.", '
                                '"Inflation-adjusted gold still 12% below 2011 peak."]')]
    ))
    post = {
        "shortCode": "abc123",
    ...
    with patch("agents.instagram_agent.AsyncAnthropic", return_value=mock_client):
    ...
    assert 2 <= len(result) <= 3, f"Expected 2-3 alternatives, got {len(result)}"
    for alt in result:
        assert "#" not in alt, f"Alternative must not contain hashtags: {alt}"
...
async def test_compliance_blocks_hashtags():
    """
    Draft containing '#gold' is blocked by the compliance checker.
    Covers: INST-06
    """
    pytest.skip("Instagram agent not yet implemented")
    ia = _get_instagram_agent()
    ...
    draft_with_hashtag = "Great insight on #gold performance this quarter."
    with patch("agents.instagram_agent.AsyncAnthropic", return_value=mock_client):
        passed = await ia.check_compliance(draft=draft_with_hashtag, client=mock_client)
...
async def test_compliance_blocks_seva():
    """
    Draft containing 'Seva Mining' is blocked by the compliance checker.
    Covers: INST-08
    """
    pytest.skip("Instagram agent not yet implemented")
    ia = _get_instagram_agent()
...
async def test_compliance_fail_safe():
    ...
    with patch("agents.instagram_agent.AsyncAnthropic", return_value=mock_client):
        passed = await ia.check_compliance(draft="Some draft text.", client=mock_client)
