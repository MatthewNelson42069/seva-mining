...
- **CORRECTION (per Phase 10 research):** `frontend/src/components/summary/SummaryCard.tsx` lines 61-75 currently HARDCODE Seva field names (`gold_news_md`, `ontario_law_md`, `ontario_stats_md`) and section titles (`"Gold News"`, `"Ontario Law"`, `"Ontario Stats"`). The assumption in 10-CONTEXT.md §code_context that Phase 9 D-08 made SummaryCard tenant-tolerant was WRONG. Phase 10 MUST add a per-company section-config map.
- **NEW file:** `frontend/src/config/companySectionConfig.ts` — exports a `SECTION_CONFIG: Record<"seva" | "juno", Array<{field: string, title: string}>>` mapping each company to its 3 sections. Seva: `[{field: 'gold_news_md', title: 'Gold News'}, {field: 'ontario_law_md', title: 'Ontario Law'}, {field: 'ontario_stats_md', title: 'Ontario Stats'}]`. Juno: `[{field: 'defence_news_md', title: 'Defence News'}, {field: 'canadian_procurement_md', title: 'Canadian Procurement'}, {field: 'world_events_md', title: 'World Events Relevant to Defence'}]`. **NOTE:** Per Phase 9 D-08 (architecture), the 3 DB columns are reused semantically — Juno's `defence_news_md` is stored in the existing `gold_news_md` column, etc. (No new Alembic migration; semantic reuse of existing fields per Phase 9 ARCHITECTURE.md D-08.) So the config maps Juno's logical field names to the existing physical columns: `[{field: 'gold_news_md', title: 'Defence News'}, {field: 'ontario_law_md', title: 'Canadian Procurement'}, {field: 'ontario_stats_md', title: 'World Events Relevant to Defence'}]`. The planner picks the exact field-mapping pattern (logical-rename vs physical-column-reuse).
- **MODIFIED:** `frontend/src/components/summary/SummaryCard.tsx` — read `useParams<{company}>().company`, look up config from `SECTION_CONFIG[company]`, render sections dynamically. ~10-15 line edit.
- `frontend/src/components/summary/SectionBlock.tsx` — UNCHANGED (Phase 8 MarkdownContent wrapper still works; only the parent SummaryCard's section list changes)
- `frontend/src/components/summary/__tests__/SummaryCard.test.tsx` — extend for per-tenant rendering test
