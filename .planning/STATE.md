...
Last activity: 2026-04-20 — quick-260420-mfy pivoted infographic+quote post formats off on-platform rendering entirely. Operator's call — weeks of fighting rendering quality (Gemini Imagen → Recharts SSR → resvg-js font bugs → Docker build brittleness) weren't worth the maintenance burden when claude.ai artifacts produces higher-quality designs. Ripped `scheduler/chart_renderer/` (Node.js service, fonts, theme.js, all components), `ChartRendererClient`, Gemini/Imagen quote renderer, `ChartSpec`+`BundleCharts`+`render_bundle_job`+`_enqueue_render_job` Python code, `RenderedImage`+`rendered_images`+`RerenderResponse` Pydantic+API+frontend types, and all image-display UI (Dialog enlarge + fetch+blob download from quick-si2). Added `scheduler/agents/brand_preamble.py` with 1267-char `BRAND_PREAMBLE` baking in full Seva look (cream #F0ECE4, navy #0C1B32/#1E3A5F/#4A7FA5, gold #D4AF37, DM Serif Display titles, Inter body, SEVA MINING wordmark, 1200x675 HTML artifact format). Content agent infographic+quote branches now emit `{suggested_headline, data_facts, image_prompt}` (image_prompt = BRAND_PREAMBLE + type-specific direction). Frontend `InfographicPreview.tsx` + `QuotePreview.tsx` rewritten to three copy-buttoned text blocks; legacy (pre-mfy) bundles show "Legacy format — regenerate" placeholder. Dockerfile slimmed from Node+Python multi-layer back to 28-line pure-Python. **Verification passed (9/9 must-haves):** 106 scheduler tests + 66 backend tests + 73 frontend tests pass, ruff + eslint clean, tsc+vite build green, zero production references to deleted symbols. 6 atomic commits: `5514db0` / `be2f7a7` / `d589c68` / `8a4fcb1` / `76f3769` / `df274c3`.
Stopped at: Railway to pick up mfy pivot on next scheduler rebuild. Post-deploy watch-list: (a) scheduler image build is pure-Python (no Node layers), (b) next content_agent cron (every 3h) produces infographic/quote bundles with `suggested_headline` + `data_facts` + `image_prompt` in draft_content (NOT `charts` array or Imagen prompt), (c) dashboard cards render three copy blocks with no broken image refs. Still pending from quick-260420-lw4: Twitter agent DB spot-check on next twitter_agent cron cycle (commits `18c0701` + `b6cee40`).
Resume file: None
