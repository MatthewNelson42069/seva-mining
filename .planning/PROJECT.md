...
🎉 **v3.1 CODE-COMPLETE 2026-05-21 — Phase 15 shipped** — Juno Weekly Viral Sweeper landed (JSWEEP-01..06 verified PASS by gsd-verifier, 6/6 requirements + 13/13 invariants). Phase 15 was the largest in v3.1 — 7 plans across 3 waves: substrate-fix extension to Phase 10 writers (D-03a — story-URL arrays now persisted in `raw_sources_jsonb`), new `JUNO_SWEEPER_SYSTEM_PROMPT` with verbatim anti-tactical clause from Phase 10 + Janes/CSIS voice + "3 content angles" task framing, new `JUNO_SWEEPER_X_QUERY` constant (11 corrected handles + 2 hashtags, ~261 chars; defence-prime cashtags excluded per anti-feature), new `scheduler/agents/juno_weekly_sweeper.py` orchestrator (588 LOC + 17 unit tests covering Phase 12 D-07 hardcoded literal + Phase 9 idempotency-includes-partial + D-05 refusal-detector reuse + D-06 helper-share from Seva + D-03 3-sub-array virality compute + D-10 byte-identical Seva), cron registration at lock 1021 Sunday 08:00 PT America/Los_Angeles env-gated by `JUNO_SWEEPER_CRON_ENABLED` (mirrors Phase 10 pattern verbatim), frontend Tab 3 short-circuit removed + cross-tenant isolation test + backend 404 cross-tenant test. Operator voice UAT APPROVED 2026-05-21 (interpretation b: code+test correctness basis; `voice_calibration_uat.md` ledger reserved for post-D-03b-backfill actual smoke fire). Frontend 178→181 (+3), backend 188→191 (+3), scheduler 323→363 (+40). Pending operator out-of-band: flip `JUNO_SWEEPER_CRON_ENABLED=true` in Railway → next Sunday 08:00 PT fires real cron. **v3.1 milestone code-complete: 4 of 4 phases shipped (12+13+14+15).**

🚧 **v3.1 Phase 14 shipped 2026-05-20** — Juno Content Calendar landed (JCAL-01..05 verified PASS by gsd-verifier, 5/5 requirements + 12/12 invariants). Tightest scope in v3.1 — 13-line deletion + 2 new test files (frontend RTL with TanStack Query key isolation + backend pytest with 4 cross-tenant 404 assertions). REQUIREMENTS.md JCAL-01 wording relaxed per D-06 with `*(D-06)*` provenance marker. Zero Seva-side changes (D-07 byte-identical contract). 3 post-deploy human-verification items queued (visual confirmation /juno/calendar renders WeeklyGrid with navy palette; DayCell autosave round-trip; live CompanySwitcher Juno↔Seva with no mid-edit bleed).
...
---
*Last updated: 2026-05-21 — **v3.1 milestone code-complete** (4 of 4 phases shipped). Phase 15 (Juno Weekly Viral Sweeper) landed at 7/7 plans; 6/6 JSWEEP requirements verified PASS; 13/13 invariants. Scheduler tests 323 → 363 (+40); backend 188 → 191 (+3); frontend 178 → 181 (+3); zero regressions. Operator voice UAT APPROVED 2026-05-21 (interpretation b code+test basis). Phase 15 cron registered but DISABLED until operator out-of-band Railway flip. Pending operator actions: (A) flip `JUNO_SWEEPER_CRON_ENABLED=true` in Railway → next Sunday 08:00 PT fires Juno weekly viral sweep cron; (B) optional post-D-03b-backfill actual smoke fire to fill voice_calibration_uat.md ledger; (C) Phase 12 actions still pending (SEVA_/JUNO_ANTHROPIC_API_KEY + STRICT flip + CI integration). Next: `/gsd:audit-milestone v3.1` (recommended before `/gsd:complete-milestone`) OR ship v3.1 as-is and start scoping v3.2.*
