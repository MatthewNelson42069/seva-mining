...
🚧 **v3.1 in progress — Phase 13 shipped 2026-05-20** — Per-company Branding landed (BRAND-01..05 verified PASS by gsd-verifier, 12/12 invariants + operator visual QA APPROVED 10/10 at 1440×900). `frontend/src/config/companyBrandConfig.ts` registry of record (mirrors `companySectionConfig.ts` pattern); `useCompanyBrand()` hook resolves URL → Zustand → 'seva' fallback. `/juno/*` now shows "Juno Industries" wordmark + "J" navy square + navy-toned palette `oklch(0.58 0.14 245)` resolved via `:root.dark[data-company='juno']` CSS override (specificity 0,2,0 beats Phase 8 `.dark` 0,1,0); `/seva/*` zero regression (D-09 byte-identical). FOWB-free transitions via `CompanyBrandEffect` single useEffect committing dataset+title+favicon atomically. Polish folded: tab title + favicon swap + bare-`/` redirect (TENANT-VISITED-v31-redux closed). Frontend tests: 168 → 175 (+2 Juno default + 5 FOWB; zero regressions). Next phase: 14 (Juno Content Calendar).
...
---
*Last updated: 2026-05-20 — v3.1 Phase 13 shipped (per-company branding; 5/5 BRAND requirements verified PASS, 12/12 invariants, operator visual QA 10/10 APPROVED at 1440×900). Frontend tests 168 → 175 (+2 Juno default + 5 FOWB; zero regressions). TENANT-VISITED-v31-redux closed as D-08c opportunistic polish. Phase 12 operator post-deploy Railway env-var config still pending. Next phase: 14 (Juno Content Calendar — JCAL-01..05).*
