...
✅ **v3.1 SHIPPED 2026-05-21** — Juno Feature Parity + Branding complete (5 phases / 19 plans / 25 requirements). Audit verdict: **`passed`** (initial verdict `tech_debt` flipped to `passed` after Phase 16 closed all 5 pre-existing carry-over items). Regression baseline at close: scheduler 363 / backend 191 / frontend 181 — all GREEN; both CI grep gates exit 0; zero v3.1-introduced tech debt. See [`milestones/v3.1-MILESTONE-AUDIT.md`](milestones/v3.1-MILESTONE-AUDIT.md) + [`milestones/v3.1-ROADMAP.md`](milestones/v3.1-ROADMAP.md). Operator activation gates queued post-archive (none block): Phase 12 per-tenant Anthropic API keys + `ANTHROPIC_RESOLVER_STRICT=true` flip + CI integration; Phase 14 three visual spot-checks at 1440×900; Phase 15 `JUNO_SWEEPER_CRON_ENABLED=true` flip → next Sunday 08:00 PT fires real Juno weekly viral sweeper cron.

After v3.1, **Juno is a complete defence-industry equivalent of Seva** — twice-daily defence intelligence brief (Tab 1, live since 2026-05-20 08:05 PT), content planner (Tab 2), weekly viral sweeper (Tab 3, cron registered awaiting operator flip), per-company branding (Juno navy `oklch(0.58 0.14 245)` + "Juno Industries" wordmark + "J" letter mark + Juno favicon), per-tenant Anthropic API key resolver, and clean cost-attribution path.

✅ **v3.0.1 shipped 2026-05-20** — v3.0 audit cleanup complete (5/5 CLEANUP requirements). Production cron LIVE — `JUNO_CRON_ENABLED=true` flipped in Railway 2026-05-20.

✅ **v3.0 shipped 2026-05-19** — multi-tenant platform live. Seva + Juno toggle operational. Audit verdict: `tech_debt` (20/20 reqs satisfied; 8 follow-ups → 5 closed in v3.0.1, 3 absorbed into v3.1).

## Next Milestone: v3.2 (TBD)

v3.1 archived 2026-05-21. Next milestone unscoped — run `/gsd:new-milestone` to define scope, questioning → research → requirements → roadmap.

**Candidates carried forward** (from v3.1 Out of Scope + v3.2+ deferrals):
...
- **OPS-DASH-v3X** — Per-tenant cost dashboard (read Anthropic + SerpAPI usage by tenant via the per-tenant key separation from Phase 12); useful operator tooling
- **Sweeper system-prompt iteration** — if v3.1 Phase 15 production-data refusal rate or voice drift surfaces issues with `DEFENCE_SWEEPER_SYSTEM_PROMPT`

**Permanent anti-features** (out of scope across all milestones):
- Operational/tactical defence intelligence (Anthropic content-policy boundary preserved)
- Equity/financial signals on defence primes
- Autoposting on any platform (single-user manual approval model preserved)
...
- ✓ Per-tenant Anthropic API key resolver — `get_anthropic_client(company_id)` with module-level cache + WARN-once-on-fallback + opt-in `ANTHROPIC_RESOLVER_STRICT` env gate; all call sites routed via resolver (no direct `AsyncAnthropic()` instantiation outside `scheduler/agents/anthropic_resolver.py`); CI grep gate enforces; backwards-compat fallback to shared `ANTHROPIC_API_KEY` when per-tenant unset (KEY-01..04) — v3.1 Phase 12
- ✓ Per-company branding via `companyBrandConfig.ts` registry pattern (extends Phase 9 D-08 `companySectionConfig.ts` precedent); Juno navy `oklch(0.58 0.14 245)` + "Juno Industries" wordmark + "J" letter mark + Juno favicon; `:root.dark[data-company='juno']` selector with specificity 0,2,0 beats Phase 8's `.dark` rule (0,1,0); `useCompanyBrand()` React hook; AppHeader + favicon adapt to active tenant; zero `if (company === 'juno')` branches; operator visual QA 10/10 PASS at 1440×900 (BRAND-01..05) — v3.1 Phase 13
- ✓ Juno Content Calendar (Tab 2) — port of v2.1 Phase 6 paper-planner UI to `/juno/calendar`; weekly Mon-Sun grid with auto-save on blur; full CRUD over Juno's `calendar_items` rows via existing `scoped_*()` helpers + `/api/{company}/calendar` router prefix already established in Phase 9; cross-tenant isolation tests pass; zero regression in Seva calendar behavior (JCAL-01..05) — v3.1 Phase 14
- ✓ Juno Weekly Viral Sweeper (Tab 3) — Sunday 08:00 PT APScheduler cron registered at lock slot 1021 (reserved in Phase 9 OPS-02), `CronTrigger(day_of_week='sun', hour=8, minute=0, timezone='America/Los_Angeles')`; 11 corrected defence-industry X handles + 2 hashtags (~261 chars within 512-char X API Basic-tier limit, defence-prime cashtags excluded per anti-feature); virality compute over Juno's `daily_summaries.raw_sources_jsonb` keys `defence_news`/`canadian_procurement`/`world_events` (D-03a substrate fix: extended Phase 10 `_build_juno_*_section` writers to persist story-URL arrays — Juno-only change, Seva untouched per D-10); Sonnet 4.6 synthesis with Janes/CSIS voice + verbatim anti-tactical clause (string-equality CI grep gate); refusal-detector pattern from Phase 10 (7 substring regex + retry-with-framing-nudge + `status='partial'`); idempotency-includes-partial fix from Phase 9 preserved; `JUNO_SWEEPER_CRON_ENABLED` env gate (parallels `JUNO_CRON_ENABLED` precedent); Tab 3 React UI renders latest sweep card with empty-state copy for first deploy; operator voice UAT APPROVED 2026-05-21 (JSWEEP-01..06) — v3.1 Phase 15
- ✓ v3.1 audit cleanup bundle — Five pre-existing carry-over items closed before archive (frontend ESLint, backend ruff, scheduler ruff, scheduler test RuntimeWarning, stale Phase-9-era `SummaryFeedPage` empty-state copy). All items pre-existing (NOT v3.1-introduced); bundled into single phase mirroring Phase 11's pattern. Verifier 5/5 PASS. Initial milestone audit `tech_debt` flipped to `passed` after Phase 16 closed (CLEAN-01..05) — v3.1 Phase 16
- ✓ Regression baseline at v3.1 close: scheduler 363 / backend 191 / frontend 181 — all GREEN; both CI grep gates exit 0 (tenant-isolation + anti-tactical-clause); zero v3.1-introduced tech debt. Audit verdict: `passed` (25/25 requirements satisfied; 7/7 E2E flows WIRED end-to-end; 0 carried-over remaining). Operator activation gates queued post-archive (none block): Phase 12 per-tenant Anthropic API keys + `ANTHROPIC_RESOLVER_STRICT=true` flip + CI integration; Phase 14 three visual spot-checks at 1440×900; Phase 15 `JUNO_SWEEPER_CRON_ENABLED=true` flip → next Sunday 08:00 PT fires real Juno weekly viral sweeper cron — v3.1 milestone COMPLETE
...
- **v3.1 Phase 12 Anthropic resolver contract (D-07):** Hardcoded `'juno'` / `'seva'` string literals at all instantiation sites (NOT dynamic `company_id` variables) — preserves grep-ability of every per-tenant call site for audit and cost-attribution clarity. Module-level cache keyed on `company_id` avoids repeated `AsyncAnthropic()` construction. WARN-once-on-fallback when per-tenant key unset (logs go to scheduler stdout for operator visibility). Opt-in `ANTHROPIC_RESOLVER_STRICT=true` env gate flips fallback from WARN to RAISE — preserved as production safety lever (operator activation step post-archive after `SEVA_/JUNO_ANTHROPIC_API_KEY` env vars set in Railway).
- **v3.1 Phase 13 branding strategy (D-03):** Per-tenant CSS-variable bridging via `:root.dark[data-company='juno']` selector (specificity 0,2,0 beats Phase 8's `.dark` 0,1,0 baseline without `!important`). `companyBrandConfig.ts` registry pattern extends the Phase 9 D-08 `companySectionConfig.ts` precedent — no `if (company === 'juno') {...}` branches anywhere in source. Brand assets (logo SVG + favicon ICO) keyed on `:company` URL segment; `useCompanyBrand()` React hook resolves the active tenant config at render time. Defence-industry colour anchor: Juno navy `oklch(0.58 0.14 245)` (authoritative, subdued — explicit rejection of Seva's gold-bull amber for visual differentiation of tenant identity).
- **v3.1 Phase 15 sweeper substrate fix (D-03a):** Phase 10 had written only diagnostic counts to `daily_summaries.raw_sources_jsonb`; sweeper needs story-URL arrays. Fixed by extending `_build_juno_defence_news_section`/`_build_juno_canadian_procurement_section`/`_build_juno_world_events_section` writers to persist arrays under keys `defence_news`/`canadian_procurement`/`world_events` — Juno-only change, Seva substrate untouched per D-10 (zero regression risk to gold-sector sweeper).
- **v3.1 Phase 15 X query design (D-04):** 11 corrected defence-industry handles + 2 hashtags totalling ~261 chars (within X API Basic-tier 512-char query limit). Defence-prime cashtags (`$LMT`/`$RTX`/`$NOC`/`$GD`/`$BA`) explicitly EXCLUDED per equity/financial-signals anti-feature carried forward from v3.0. Tier-1 handles include `@defense_news`, `@CDAInstitute`, `@CanadianForces`, `@JanesINTEL` (4 handles corrected from initial planning where naive spellings were broken); Tier-2 Canadian-defence-journalism handles `@DavePerryCGAI` + `@Murray_Brewster` added during research.
- **v3.1 Phase 16 parallel-execution lesson:** Wave-based parallel executor agents share `.git/index` when committing concurrently with `--no-verify`. Phase 16 plans 04 + 05 race-bundled their frontend code into a single commit `5fc45f0` under 04's attribution. Zero functional impact (both plans landed atomically), but documented in both `SUMMARY.md` files as a known executor edge case. Future executors: sequence file-disjoint commits within the same wave OR avoid `--no-verify` so pre-commit hooks serialize via filelock.
...
---
*Last updated: 2026-05-21 — **v3.1 milestone ARCHIVED** (5 of 5 phases shipped + audit verdict `passed` + tagged `v3.1`). Roadmap collapsed; phase artifacts moved to `milestones/v3.1-phases/`; requirements archived to `milestones/v3.1-REQUIREMENTS.md`; fresh `REQUIREMENTS.md` will be created via `/gsd:new-milestone` when v3.2 is scoped. Operator activation gates queued post-archive (none block): (A) flip `JUNO_SWEEPER_CRON_ENABLED=true` in Railway → next Sunday 08:00 PT fires Juno weekly viral sweep cron; (B) set `SEVA_ANTHROPIC_API_KEY` + `JUNO_ANTHROPIC_API_KEY` + flip `ANTHROPIC_RESOLVER_STRICT=true`; (C) optional Phase 14 visual spot-checks at 1440×900. Next: `/gsd:new-milestone` to scope v3.2.*
