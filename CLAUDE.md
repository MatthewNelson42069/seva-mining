...
A single-agent AI system that monitors the gold sector via news feeds 24/7, drafts engagement content, and surfaces everything to a web dashboard for manual approval. The Content Agent handles research, scoring, and drafting — you review, approve, copy, and post. Nothing is ever posted automatically. A trimmed morning-digest job sends daily digests and alerts via WhatsApp.
...
- **Stack**: FastAPI (Python) backend, React + Tailwind CSS frontend, APScheduler in separate worker process (three scheduled jobs: `content_agent`, `morning_digest`, `gold_history_agent`)
